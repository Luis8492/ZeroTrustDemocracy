...
本サービスで表示する会議録は、以下の公開情報を取得・構造化したものです。

- [世田谷区議会（区公式ホームページ）](https://www.city.setagaya.lg.jp/gikai/index.html)
- [世田谷区議会 会議録検索システム](https://kugi.city.setagaya.tokyo.jp/voices/)

本サイトは有志による試作プロジェクトであり、**世田谷区および世田谷区議会の
公式サービス・公式見解ではありません**。会議録を機械的に質疑応答ペアへ
切り出して表示しているため、区切り方や発言者の対応付けに誤りを含む
可能性があります。正確な内容は必ず原文（出典元）でご確認ください。

会議録データの取得対象は **直近 4 年分** に限定しています
（[`utils/date_filter.py`](utils/date_filter.py) の `DEFAULT_MAX_AGE_YEARS`）。

誤った表示の指摘・削除依頼などは [GitHub Issues](https://github.com/Luis8492/ZeroTrustDemocracy/issues) までお願いします。
